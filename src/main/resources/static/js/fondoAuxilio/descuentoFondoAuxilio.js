...
function guardarDosificacion(){
    ...
    $.ajax({
        type: "POST",
        url: "fondoAuxilio/guardarDescuentoAuxilio",
...
function bloqueaBotonGuardar(id_trabajador){
    $.ajax({
        type: "GET",
        url: 'fondoAuxilio/listarCuentaPorTrabajador/'+id_trabajador,
        dataType: 'json',
        success: function (data) {
            
            if (data !== undefined) {
                if (data[0].id_auxilio_cuentas_cobrar !== null && data[0].estatus === 1) {
                    $('#id_cuenta_por_cobrar').val(data[0].id_auxilio_cuentas_cobrar);
...
function eliminaCuenta(){
    ...
    $.ajax({
        type: "POST",
        url: "fondoAuxilio/estatusCuentaPorCobrar/" + trabajador_id,
...
$("#saldarCuentaModal").submit(function (e) {
    ...
    var datos = {"monto_pago": monto_pago, "trabajador_id": trabajador_id, "periodo_pago":periodo_saldar_cuenta, "saldo_deuda":saldo_actual, "cuentas_Por_Cobrar":{"id_auxilio_cuentas_cobrar":id_cuenta_por_cobrar}, "justificacion":justificacion};
    
    $.ajax({
        type: "POST",
        url: "fondoAuxilio/guardarRegistroPagosDeudas",
        dataType: 'json',
        contentType: "application/json",
        data: JSON.stringify(datos),
        success: function (data) {
...
function actualizaEstatusCuentaSaldada(id_cuenta_por_cobrar,trabajador_id){
    $.ajax({
        type: "POST",
        url: "fondoAuxilio/estatusCuentaSaldada/" + id_cuenta_por_cobrar,
...
function fondoAuxilioAdmin() {
    window.location.href = 'fondoAuxilioAdmin';
}
