...
//Obtiene el id del banco, con base en el trabajador_id
function obtenIdBanco(trabajador_id){
    
    $.ajax({
        type: "GET",
        url: "trabajador/buscarCuentaidTrabajador/" + trabajador_id,
        dataType: 'json',
        success: function (data) {
            detectaBanco(data.data.banco_id);
        },
        error: function (e) {
            alert(e);
        }
...
//Detecta el banco diferente a Banorte
function detectaBanco(id_banco){
    ...
    $.ajax({
        ...
        dataType: 'json',
        success: function (data) {
            if (data.data.nombre_banco !== "Banorte") {
                $("#alertaBancoModal").modal('show');
                var mensaje_banco = document.getElementById('mensaje_banco');
                mensaje_banco.textContent = "Se ha detectado que la cuenta del Trabajador es de banco: " + data.data.nombre_banco + ", se cobrará un 6.96%";
                $('#impuesto_banco').val(6.96);
            }
...
//EN CASO QUE EL TRABAJADOR TENGA OTRO BANCO, SE GENERA OTRO REGISTRO AL MISMO TIEMPO CON EL IMPORTE DE 6.96 PARA DESCONTARLO EN LA SIGUIENTE QUINCENA
function guardarimpuesto(){
    ...
    var saldo_inicial = "6.96";
    var saldo_actual = saldo_inicial;
    var monto_pago = saldo_inicial;
    var numero_periodos_pago = "1";
    ...
    var datos = {"trabajador":{"id_trabajador": trabajador_id}, "cat_Tipo_Nomina": {"id_tipo_nomina":id_nomina}, "saldo_inicial" :saldo_inicial, "saldo_actual":parseFloat(saldo_actual), "monto_pago":monto_pago,
                ...
                "periodo_inicial":periodo, "monto_criterio_pago":numero_periodos_pago,"saldo_actual_pr":saldo_inicial};
    $.ajax({
        type: "POST",
        url: "creditoPersonal/guardarDescuentoPersonal",
        data: JSON.stringify(datos),
        contentType: "application/json",
        success: function (data) {
            toastr['success'](data.data, "Impuesto Guardado Correctamente");
...
// ASIGNA EL PERIODO CORRESPONDIENTE A LA FECHA DE SOLICITUD DEL DESCUENTO PERSONAL
...
function bloqueaBotonGuardar(id_trabajador){
    $.ajax({
        type: "GET",
        url: 'creditoPersonal/listarCuentaPorTrabajadorPersonal/'+id_trabajador,
        dataType: 'json',
        success: function (data) {
            
            if (data !== undefined) {
                if (data[0].id_personal_cuentas_cobrar !== null && data[0].estatus === 1) {
                    $('#id_cuenta_por_cobrar').val(data[0].id_personal_cuentas_cobrar);
...
function eliminaCuenta(){
    ...
    $.ajax({
        type: "POST",
        url: "creditoPersonal/estatusPersonalCuentaPorCobrar/" + trabajador_id,
...
$("#saldarCuentaModal").submit(function (e) {
    ...
    var datos = {"monto_pago": monto_pago, "trabajador_id": trabajador_id, "periodo_pago":periodo_saldar_cuenta, "saldo_deuda":saldo_actual, "cuentas_Por_Cobrar":{"id_personal_cuentas_cobrar":id_cuenta_por_cobrar}, "justificacion":justificacion};
    
    $.ajax({
        type: "POST",
        url: "creditoPersonal/guardarRegistroPagosDeudasPersonal",
        dataType: 'json',
        contentType: "application/json",
        data: JSON.stringify(datos),
        success: function (data) {
...
function actualizaEstatusCuentaSaldada(id_cuenta_por_cobrar,trabajador_id){
    $.ajax({
        type: "POST",
        url: "creditoPersonal/estatusCuentaSaldada/" + id_cuenta_por_cobrar,
...
// LIMITA LA CANTIDAD DE PERIODOS INGRESADA EN LOS INPUTS
