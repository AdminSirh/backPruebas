...
const tabs = (id_trabajador) => {
    $('#myTab').html("<ul class='nav nav-tabs' role='tablist'>" +
            "<li class='btn btn-primary' onclick=descuentoPersonal('" + id_trabajador + "')>1.- Descuentos Personales</li>&nbsp;" +
...
const descuentoPersonal = (id_trabajador) => {
    window.location.href = 'descuentoPersonal?id_trabajador=' + id_trabajador;
};

const cuentaActiva = (id_trabajador) => {
    window.location.href = 'creditoPersonalCuentaActiva?id_trabajador=' + id_trabajador;
};

const pagos = (id_trabajador) => {
    window.location.href = 'creditoPersonalPagos?id_trabajador=' + id_trabajador;
};
