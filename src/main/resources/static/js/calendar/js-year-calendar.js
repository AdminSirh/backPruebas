//Para colocar cuadrado o círculo dependiendo del personal que creó la incidencia
...
(function (global, factory) {
    ...
    var Calendar = /*#__PURE__*/function () {
        ...
        _createClass(Calendar, [{
                ...
                //Renderiza el día con la incidencia, lo voy a utilizar para renderizar un cuadrado o un círculo según el área
                key: "_renderDataSourceDay",
                value: function _renderDataSourceDay(elt, currentDate, events) {
                    var parent = elt.parentElement;
                    // Llamar a createRelationArray con el elemento actual, esta funcion se llama para renderizar cada día y repite el datasource cada vez, solo cambia el parent
                    createRelationArray(parent, this._dataSource, events.length);
                ...
                value: function _applyEvents() {
                    ...
                    cells.forEach(function (cell) {
                        ...
                        /* Range selection 
                         * Removed because it causses issues with our application logic (Sirh)
                         * */
...
//Esta función busca el rol del usuario que creó la incidencia, esta se llama cada que se renderizan los días de la incidencia
function createRelationArray(elementoDia, dataSource, longituDeEventos) {
    const meses = [
        "Enero",
        "Febrero",
        "Marzo",
    ...
    let parentArray = [];
    //Se guardan todos los días con eventos en un arreglo
    parentArray.push(elementoDia);
    //Si ya se superó el último evento a registrar entonces se procede a la lógica de asignación de los elementos  
    if (longituDeEventos > 1) {
        return;
    }
    const diaFecha = dataSource[0].startDate.getDate();
    // Se crea un arreglo vacío para relacionar los días con los eventos
    const relaciones = [];
    // Iteramos sobre los elementos de parentArray y dataSource
    parentArray.forEach((parentElement) => {
        // Obtenemos el número del día del elemento de parentArray y el mes que lo contiene
        const numeroDiaParent = parseInt(parentElement.children[0].textContent, 10);
        const elementoPadreMes = parentElement.parentNode.parentNode;
        const mesParent = elementoPadreMes.querySelector('th.month-title').textContent.trim();
        // Se buscan los eventos que coinciden con el número del día y el mes, solo para el año en curso
        const eventosRelacionados = dataSource.filter(data => {
            const fechaInicio = new Date(data.startDate);
            // Se compara el día y el mes del data con el día y el mes del elemento parent
            return fechaInicio.getDate() === numeroDiaParent &&
                    meses[fechaInicio.getMonth()] === mesParent;
        });
        // Se crea un objeto que contiene los eventos relacionados con su elemento padre
        const relacionDiaEvento = {
            dia: parentElement,
            eventos: eventosRelacionados
        };
        // Se añade la relación al arreglo de todas las relaciones de eventos
        relaciones.push(relacionDiaEvento);
    });
    //Se llama a la función que consulta el adming log mandándole las relaciones correspondientes
    searchUserRoleAdminLog(relaciones);
}

const searchUserRoleAdminLog = (relaciones) => {
    //Listado de operaciones a buscar realizadas por los usuarios con los roles de nómina o personal
    const operacionesABuscar = ['guardarIncidenciaTrabajador', 'guardarIncidenciaTrabajadorVacaciones'];
    //const operacionABuscar = 'guardarIncidenciaTrabajadorVacaciones';
    ...
    operacionesABuscar.forEach(operacionABuscar => {
        ajaxCallForEachOperationRelatedToKardex(operacionABuscar, startDate, endDate, relaciones);
    });

};

const ajaxCallForEachOperationRelatedToKardex = (operacionABuscar, startDate, endDate, relaciones) => {
    $.ajax({
        type: "GET",
        ...
        success: function (dataAdminLog) {
            ...
            dataAdminLog.forEach(function (elemento) {
                ...
                // Se recorren cada una de las relaciones del arreglo relaciones
                relaciones.forEach(relacion => {
                    const dia = relacion.dia;
                    const eventos = relacion.eventos;
                    eventos.forEach(evento => {
                        if (evento.id === idIncidenciaGenerada) {
                            if (elemento.creators_role === "ROLE_PERSONAL") {
                                //console.log('generando circulo para id' + evento.id);
                                generarCirculo(dia);
                            } else if (elemento.creators_role === "ROLE_NOMINA") {
                                //console.log('generando cuadrado para id' + evento.id);
                                generarCuadrado(dia);
                            }
                        }
                    });
...
const generarCirculo = (elementoDia) => {
    $(elementoDia).wrapInner('<div class="circle-container"></div>');
...
const generarCuadrado = (elementoDia) => {
    $(elementoDia).wrapInner('<div class="square-container"></div>');
    $('.square-container').css({
        'position': 'relative',
        'overflow': 'hidden'
    }).prepend('<div class="square"></div>');

    // Estilos CSS para el cuadrado
    $('.square').css({
        'position': 'absolute',
        'top': '50%',
        'left': '50%',
